...
zplayer.prototype.init = function() {
	...
	ht+= '			<div class="zplayer-controller">'
		...
		ht += this.option.showlrc ? '<i class="lrc-icon icon icon-list-alt"></i>' : '';
		ht +='					</div>'
			+'				</div>'
			+'			</div>'
			+'		</div>'
    ...
    if(!this.isMobile){
    	this.volumeIcon.addEventListener("click", function() {
	        ...
	        p.volumeIcon.className = 1 === p.audio.volume ? "volume-icon icon icon-volume-up" : "volume-icon icon icon-volume-down",
    ...
    if(this.option.showlrc){
    	this.lrcIcon.addEventListener("click", function() {
    		this.classList.contains("lrc-hide") ? (p.element.classList.add("zplayer-withlrc"),this.classList.remove("lrc-hide"),this.style.opacity=1,p.element.getElementsByClassName("zlrc")[0].style.display="block") : (p.element.classList.remove("zplayer-withlrc"),this.classList.add("lrc-hide"),this.style.opacity=0.5,p.element.getElementsByClassName("zlrc")[0].style.display="none");
	    })
    }
    if(this.option.fixed){
    ...
    function playSwitch(i) {
        ...
        p.playedTime = setInterval(function() {
            ...
            p.option.showlrc && p.updateLrc.call(p),
